	...
	if %cfs-jrnl% equ true (
		if %RUN% equ true (
			...
			for /L %%i in (1,1,%id%) do (
				...
				call:cmd %FS%\log "%EUJ% /DevicePath:!drivesuc[%%i]!: /OutputPath:!tmp!"
...
:malware-contd
	...
	if %cmal-startup% equ true (
		if %RUN% equ true (
			call:header "user startup items" "copying"
		...
		if %cmal-vss% equ true (
			for /L %%i in (1,1,%ip%) do (
				if %RUN% equ true (
					call:cmdn %MAL%\log "xcopy vss*\Startup %MAL%\Startup-*-* /E /C /I /F /H /Y"
				) else set /A itt+=1
...
:registry
	...
	if %creg-user% equ true (
		...
		) else (mkdir %REG%\user & set /A it+=1, itt+=2*%ip%)
		if %creg-vss% equ true (
			for /L %%i in (1,1,%ip%) do (
				for /L %%a in (1,1,%iv%) do (
					if /I %UPROFILED% equ !vsscd[%%a]! (
...
:header
	...
	set /A in+=1
	set "tmp=collecting"
	if not "%~2" equ "" set "tmp=%~2"
	echo  %datetime% %in% out of !it!: "%tmp% %~1"
	goto:eof
...
:cmdn
	call:date
	set /A inn+=1
	echo  %datetime% running %inn% out of %itt%: %2 >> %LOG%
	echo  %datetime% running %inn% out of %itt%: %2 >> %SYSROOTT%
	echo. >> %~1.txt
...
:date
	:: intrusive
	for /F "tokens=3 delims= " %%i in ('reg query "HKCU\Control Panel\International" 2^>^&1 ^| findstr "sShortDate"') do set "ssShortDate=%%i"
	...
	if %f% equ 0 (
		...
		if !cvss! equ true (
			...
			if !iv! equ 1 (
				...
				set tmp=
				for /F "delims=0123456789" %%i in ("!cvss-limit!") do set "tmp=%%i"
				if defined tmp set cvss-limit=3
		...
		:: for /F "delims=: tokens=1,*" %%i in ('fsutil fsinfo drives') do (for %%a in ("%%j") do (...))
		set tmp=
		if !cdrives-limit! equ all set tmp=true
		if !cdrives-limit! equ false set tmp=true
		if defined tmp (

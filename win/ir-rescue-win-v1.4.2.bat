	...
	if %cfs-jrnl% equ true (
		if %RUN% equ true (
			...
			for /L %%i in (1,1,%id%) do (
				...
				%EUJ% /DevicePath:!drivesuc[%%i]!: /OutputPath:"!tmp!" >> %FS%\log 2>&1
...
:filesystem-contd
	if %cfs-boot% equ true (
		if %RUN% equ true (
			call:header "disk zero boot sector" "dumping"
			call:cmdn %FS%\log "%MMCAT% \\.\PhysicalDrive0 0"
			%MMCAT% \\.\PhysicalDrive0 0 > %FS%\boot 2>&1
		) else (set /A it+=1, itt+=1)
	)
	if %cfs-table% equ true (
		if %RUN% equ true (
			call:header "disk zero partition table information"
			call:cmd %FS%\boot-table "%MMLS% -Brv \\.\PhysicalDrive0"
		) else (set /A it+=1, itt+=1)
	)
...
:init
	...
	if %f% equ 0 (
		...
		call:rconff filesystem-table cfs-table !cfs-all! !cfs!
...
:end
	...
	call:cleanf %FS%\boot %FS%\$MFT-*.bin %FS%\$LogFile-*.bin %FS%\$UsnJrnl_$J-*.bin
...
:notes
	...
	:: load pmem driver and run strings against \\.\pmem
	:: tools are unable to open the device; mklink does not work
	:: process command line arguments
	:: rekall live
	:: WMIC path win32_process get Caption,Processid,Commandline
	:: WMIC /OUTPUT:C:\Process.txt path win32_process get Caption,Processid,Commandline
	:: https://isc.sans.edu/forums/diary/Searching+live+memory+on+a+running+machine+with+winpmem/17063/
	:: Windows Scripting
	:: PowerShell and WMI

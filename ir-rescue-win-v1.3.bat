...
:memory
	...
	if %cmem-pf% equ true (
		if %RUN% equ true (
			...
			ren %MEM%\%tmem-pf:~3% %tmem-pf:~3,-4%-live.%tmem-pf:~-3%
		) else (
			for /F "skip=2 tokens=1-3" %%i in ('reg query "HKLM\SYSTEM\CurrentControlSet\Control\Session Manager\Memory Management" /V PagingFiles') do (set tmem-pf=%%k)
			set /A it+=1
		)
	...
	if %cmem-hf% equ true (
		if %RUN% equ true (
			call:cmd %MEM%	log ^
				"%RCP% /FileNamePath:%SystemDrive%\hiberfil.sys /OutputPath:%MEM%"
			ren %MEM%\hiberfil.sys hiberfil-live.sys
		) else (set /A it+=1)
		if %cmem-vss% equ true (
			for /L %%i in (1,1,%iv%) do (
	...
	if %creg-user% equ true (
		if %RUN% equ true (
			for /L %%i in (1,1,%ip%) do (
				call:cmdn %REG%	log ^
					"%RCP% /FileNamePath:!uprofiles[%%i]!\NTUSER.dat /OutputPath:%REG%\user\"
				%RCP% /FileNamePath:"!uprofiles[%%i]!\NTUSER.dat" /OutputPath:%REG%\user\ >> %REG%\log.txt 2>&1
				call:cmdn %REG%	log ^
					"%RCP% /FileNamePath:!uprofiles[%%i]!\AppData\Local\Microsoft\Windows\UsrClass.dat /OutputPath:%REG%\user\"
...
:filesystem
	...
	if %cfs-ads% equ true (
		if %RUN% equ true (
			for /L %%i in (1,1,%id%) do (
				call:cmd %FS%	log ^
					"%ADS% /FolderPath !drivesuc[%%i]!:\ /ScanSubfolders 1 /SubFolderDept 0 /ShowZeroLengthStreams 1 /scomma %FS%\ads-!driveslc[%%i]!.csv"
			)
		) else set /A it+=%id%
	)
	if %cfs-mft% equ true (
...
:malware
	if %cmal-pf% equ true (
		if %RUN% equ true (
			call:xcp %MAL%	log	"%SystemRoot%\Prefetch\*.pf" "%MAL%\Prefetch-live"
			call:cmdn %MAL%	log ^
				"%WPV% /prefetchfile %SystemRoot%\Prefetch\*.pf /sort ~7 /scomma %MAL%\Prefetch-live\*.csv"
			for /F "tokens=*" %%i in ('dir /B /S %SystemRoot%\Prefetch\*.pf') do (
				call:cmdr %MAL%	log	"%WPV% /prefetchfile %%i /sort ~7 /scomma %MAL%\Prefetch-live\%%~ni.csv"
			)
		) else set /A it+=2
		if %cmal-vss% equ true (
			for /L %%i in (1,1,%iv%) do (
				if /I %SYSROOTD% equ !vsscd[%%i]! (
					if %RUN% equ true (
						call:xcp %MAL%	log ^
							"!vssc[%%i]!%SYSROOTP%\Prefetch\*.pf" "%MAL%\Prefetch-!vsscf[%%i]!\"
						call:cmdn %MAL%	log ^
							"%WPV% /prefetchfile !vssc[%%i]!%SYSROOTP%\Prefetch\*.pf /sort ~7 /scomma %MAL%\Prefetch-!vsscf[%%i]!\*.csv"
	...
	if %cmal-cache% equ true (
		if %RUN% equ true (
			...
			ren %MAL%\cache\RecentFileCache.bcf RecentFileCache-live.bcf
			ren %MAL%\cache\Amcache.hve Amcache-live.hve
			...
			for /L %%i in (1,1,%iv%) do (
				if /I %SYSROOTD% equ !vsscd[%%i]! (
					if %RUN% equ true (
						...
						ren %MAL%\cache\RecentFileCache.bcf RecentFileCache-!vsscf[%%i]!.bcf
						ren %MAL%\cache\Amcache.hve Amcache-!vsscf[%%i]!.hve
					) else set /A it+=2
				)
			)
		)
...
:malware-contd
	...
	if %cmal-hid% equ true (
		if %RUN% equ true (
			call:cmdn %MAL%\hidden	dir-hidden		"dir /A:HS /O:D /Q /T:W *"
			call:cmdn %MAL%\hidden	attrib-hidden	"attrib /L *"
			for /F %%i in (%CFG%\nonrecursive.txt) do (
			...
			call:cmdn %MAL%\hidden	attrib-hidden	"attrib /D /L /S *"
	...
	if %cmal-startup% equ true (
		if %RUN% equ true (
			call:cmdn %MAL%	log ^
				"xcopy *\Startup %MAL%\Startup-*-live /E /C /I /F /H /Y"
			for /L %%i in (1,1,%ip%) do (
...
:activity
	...
	if %cactiv-usb% equ true (
		if %RUN% equ true (
			call:cmd %ACTIV%	log ^
				"%USB% /DisplayDisconnected 1 /DisplayNoPortSerial 1 /DisplayNoDriver 1 /RetrieveUSBPower /MarkConnectedDevices 1 /AddExportHeaderLine 1 /sort ~10 /scomma %ACTIV%\usb.csv"
		) else set /A it+=1
	)
...
:cpf
	call:cmd %~1	%~2			"copy %~3 %~4 /Y"
	goto:eof

:xcp
	call:cmd %~1	%~2			"xcopy %~3 %~4 /C /I /F /H /Y"
	goto:eof
...
:date
	:: invasive
	for /F "tokens=2*" %%i in ('reg query "HKCU\Control Panel\International" /V sShortDate^|find "REG_SZ"') do set "ssShortDate=%%j"
...
:init
	...
	if %f% equ 0 (
		...
		for /L %%i in (1,1,!iu!) do (
			for /F "skip=1" %%a in ('%PSG% -nobanner -accepteula !users[%%i]!') do (
				set "usid[%%i]=%%a"
			)
		)
			...
			if !iv! equ 1 (
				...
				for /F "tokens=3-4 delims= " %%i in ('vssadmin list shadows ^| findstr /C:"Original Volume" /C:"Shadow Copy Volume"') do (
					...
					if !tmpn! equ 0 (
						...
						mklink /D %TEMPIR%\vss-!tmp:~46!-!tmpv:~1,1! %%j\
					) else (
						set tmpv=%%i
					)
				)
...
:screenshot
	if %cscreenshot% equ true (
		set /A is+=1
		call:cmdln	"%LOG%"	"%NIRC% savescreenshotfull %META%\screenshot-%is%.jpg"
	)
	goto:eof
...
:packf
	for %%i in (%*) do (
		if exist .\%%i (
			call:cmdln	"%LOG%"	"%ZIP% a -tzip -ssc .\%%i.zip .\%%i"
		)
	)
	goto:eof
...
:packff
	if exist .\%~1 (
		call:cmdln	"%LOG%"	"%ZIP% a -tzip -ssc .\%~2.zip .\%~1"
	)
	goto:eof
...
:packd
	...
	if exist %~2 (
		...
		if "%~1" equ "zip" (
			call:cmdln	"%LOG%"	"%ZIP% a -tzip -ssc -r .\!tmp!.zip .\%~2"
		) else (
			call:cmdln	"%LOG%"	"%ZIP% a -t7z -ssc -r -pinfected .\!tmp!.7z .\%~2"
		)
	)
	goto:eof
